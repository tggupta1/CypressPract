...
conduitTests(urls, () => {
    describe('Conduit application test cases', () => {
        ...
        xit('Verify user tries to login with incorrect username', () => {
            cy.get('a.nav-link[href="#/login"]').click()
            cy.title().should('equal', 'Sign in — Conduit')
        ...
        xit('Verify user tries to login with incorrect password', () => {
            cy.get('a.nav-link[href="#/login"]').click()
            cy.title().should('equal', 'Sign in — Conduit')
            cy.get('input.ng-valid-email').type('shivanigoel@gmail.com')
        ...
        xit('Verify user tries to login without password', () => {
            cy.get('a.nav-link[href="#/login"]').click()
            cy.title().should('equal', 'Sign in — Conduit')
            cy.get('input.ng-valid-email').type('shivanigoel@gmail.com')
            //cy.get(`input.ng-pristine[type='password']`).type('password125')
        ...
        xit('Verify user tries to login without username and password', () => {
            cy.get('a.nav-link[href="#/login"]').click()
            cy.title().should('equal', 'Sign in — Conduit')
            cy.get('input.ng-valid-email').type('shivanigoel@gmail.com')
        ...
        xit('Verify user tries to login with incorrect format of email address', () => {
            cy.get('a.nav-link[href="#/login"]').click()
            cy.title().should('equal', 'Sign in — Conduit')
